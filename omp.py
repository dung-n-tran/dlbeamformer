...
def omp(X, y, nonneg=True, ncoef=None, maxit=200, tol=1e-3, ztol=1e-12, verbose=True):
    ...
    # initialize things
    X_transpose = X.transpose().conjugate()                       # store for repeated use
    #active = np.array([], dtype=int)         # initialize list of active set
    active = []
    coef = np.zeros(X.shape[1], dtype=np.complex64) # solution vector
    residual = y                             # residual vector
    ypred = np.zeros(y.shape, dtype=np.complex64)
